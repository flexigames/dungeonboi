...
export default class HUD {
  constructor(player) {
    ...
    this.healthSprites = times(player.maxHealthLimit).map((i) =>createHudSprite('ui_heart_full', 20 + i * 32, 20))
  ...
  createXpBar() {
    ...
    const middle = createHudSprite("progressbar_middle", 27, posY,  { stage: state.app.stage })
    middle.width = this.progresssMaxWidth
    createHudSprite("progressbar_right", progressBarWidth + 7, posY,  { stage: state.app.stage })
  ...
  update() {
    ...
    this.xpBar.width =
      ((this.player.xp - this.player.previousXpLimit) / (this.player.xpLimit - this.player.previousXpLimit)) *

...
export default function generateDungeon(opts = {}) {
  ...
  addTilesForWrongConstellations()
  ...
  function addTilesForWrongConstellations() {
    const world = createWalkableLevelMap(tiles, worldWidth, worldHeight)

    function getWalkable(x, y) {
      return Boolean(world[y] && world[y][x])
    }

    world.forEach((row, y) => row.forEach((walkable, x) => {
      if (
        !walkable &&
        (getWalkable(x, y - 1) && (getWalkable(x, y + 1))) ||
        (getWalkable(x, y - 1) && (getWalkable(x, y + 2))) ||
        (getWalkable(x, y - 2) && (getWalkable(x, y + 1))) ||
        (getWalkable(x - 1, y) && (getWalkable(x + 1, y))) ||
        (getWalkable(x - 1, y) && (getWalkable(x + 2, y))) ||
        (getWalkable(x - 2, y) && (getWalkable(x + 1, y)))
      ) {

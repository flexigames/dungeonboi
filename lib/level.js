import { renderEntities, createEntity, clearEntities } from "../lib/entities"
...
export function createLevel(tiles, player, opts = {}) {
  ...
  spawnEntities(
    [
      { create: (x, y) => Enemy.createRandom(x, y), count: enemyCount },
      {
        create: (x, y) => new Weapon(x, y, { sprites: "weapon_regular_sword" }),
        count: weaponCount,
      },
      { create: (x, y) => new Potion(x, y), count: potionCount },
      { create: (x, y) => new Chest(x, y), count: chestCount },
      { create: (x, y) => new Ladder(x, y), count: 1 },
    ],
    randomizedTiles
  )
}
